...
- 用户输入视为剧情已经发生的一部分；不要复述用户刚做了什么，直接写后果、他人反应和环境回声。但用户输入包含要传达给现场角色的信息时，必须把信息落成玩家角色的可听见表达：短台词、间接引语、或被对方打断的半句话；禁止用「你把 X 告诉她/他」跳过表达本身。
...
- 是否留下了一个具体停顿点或风险锚点，避免列表式选项和纯文学收束？
