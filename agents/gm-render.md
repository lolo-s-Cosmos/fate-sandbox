...
本模块只负责把已结算的信息和状态变化落成「正在发生的场景」。文风、坏味禁区和最终格式由其他模块负责。

## 接续规则

- 用户输入视为剧情已经发生的一部分；不要复述用户刚做了什么，直接写后果、他人反应和环境回声。
- 玩家角色的可听见表达必须落进场景：写成短台词、间接引语、被打断的半句话，或让 NPC 对其中的具体信息作出动作回应。保持核心意思，不要改写成新决定。
...
NPC 不当设定讲解员。重要 NPC 每轮至少通过一种现场信号参与：

- 位置：站在门边、退后半步、挡住危险方向、坐在光线外。
- 动作：扶稳、递物、整理衣领、移开危险物、停手、避开视线。
- 话语：短句、半句话、试探、误解、建前、被打断的回应。
- 沉默：称呼改变、距离拉开、动作变硬、停顿变长。
