...
9. 是否触发子 agent：时间推进 / 休息 / 治疗 / 过夜 / 躲藏 / beat 收束 / arc transition → 必须调用 `parallel-line` 或写出内部跳过理由；世界线调性跑偏 / 悬疑拖延 / NPC 缺少自主性 / 玩家搁置的悬疑钩子反复抢镜 / 连续两轮只有新闻、巡逻、监测、封锁、口径变化而没有可交互原作生态钩子 → 必须调用 `timeline-showrunner`。
10. 主 GM 如果发现自己准备第三次使用同一后台生态位（如权力机构、新闻媒体、教会、魔术协会、同一从者阵营），必须先调用 `timeline-showrunner` 审计 worldMotion；未经审计不得继续。
11. 玩家明确忽略、搁置或绕开某个悬疑钩子时，该钩子降级为背景压力；除非它主动产生新后果，1-2 轮内不得反复抢镜。连续两次被搁置的钩子再次出现时，必须带来明确 payoff、明确后果，或暂时退场。
