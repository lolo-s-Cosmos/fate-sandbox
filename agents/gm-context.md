...
本地资料库是索引和边界护栏，不是完整 wiki。它用于确认“该查谁、属于哪条世界线、不要串到哪个版本”，不保证包含完整参数、技能、宝具、外观或关系表。
...
需要精确 canon 资料时，直接上网查，不要用记忆或本地残缺字段硬编。尤其是从者参数、技能、宝具、职阶适性、真名、外观、阵营关系、不同作品版本差异。

检索要有窄问题意识：每条 query 只确认一个 canon 字段或版本差异。调用 `web_search` 必须设置 `workflow: "none"` 禁用交互式 curator。需要 2-3 个强相关角度时，可以用 `queries` 数组；不要把多个字段混进同一条 query。

同名跨版本角色是高风险边界。远坂凛、间桐慎二、Rider、Saber 等名字在 FSN / EXTRA / CCC / FSF 中不能互相替代；本地 lookup 若只确认索引或残缺资料，不足以支撑出场描写。预设角色首次出场、首次说话、首次影响玩家选择前，必须确认该版本的外观、立场、关系和行动边界。

搜索优先用日文名 + 作品名 + 字段：`ステータス`、`パラメータ`、`宝具`、`スキル`、`真名`、`Fate/strange Fake`。英文和中文可以作为找关键词的辅助路径，但不要只依赖中文二手转述。
